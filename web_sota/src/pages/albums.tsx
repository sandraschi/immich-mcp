    ...
    const fetchAlbums = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          const msg =
            (body as { detail?: string }).detail || res.statusText || "Failed to fetch albums";
          throw new Error(msg);
        }
        ...
        {albums.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {albums.map((album) => (
              <div
                key={album.id}
                className="group relative p-4 bg-slate-900/80 hover:bg-slate-800 rounded-xl border border-slate-700/50 transition-all cursor-pointer"
              >
