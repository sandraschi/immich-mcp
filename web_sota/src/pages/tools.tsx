import { AlertCircle, Code2, Loader2, Wrench } from "lucide-react";
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-400">
            Explore the available Immich MCP tools and portmanteau structures.
          </p>
        </div>
      </div>
        ...
        {tools.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {tools.map((tool) => (
              <div
                key={tool.name}
                className="group relative p-5 bg-slate-900/80 hover:bg-slate-800 rounded-xl border border-slate-700/50 transition-all flex flex-col h-full"
              >
