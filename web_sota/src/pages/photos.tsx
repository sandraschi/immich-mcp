            ...
            <div className="bg-slate-950/30 border border-slate-800/50 rounded-2xl p-6 backdrop-blur-sm min-h-[500px]">
                ...
                {!loading && photos.length === 0 && !error && (
                    <div className="flex flex-col items-center justify-center min-h-[400px] text-slate-500 py-24 border-2 border-dashed border-slate-800/50 rounded-2xl bg-slate-900/10">
                        ...
                        <p className="text-lg font-medium">{mode === 'search' ? 'No matching photos.' : 'No photos in library yet.'}</p>
                        <p className="text-sm opacity-60 mt-2 text-center max-w-md">{mode === 'search' ? 'Try a different search or click Show all.' : 'Add photos via Immich or run a scan.'}</p>
                    </div>
                )}
                                    ...
                                    {monthPhotos.map(photo => (
                                        <div key={photo.id} className="group relative aspect-square bg-slate-900 rounded-2xl overflow-hidden border border-slate-800/50 hover:border-primary/50 transition-all duration-500 hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)] hover:shadow-primary/5 hover:-translate-y-2 cursor-pointer shadow-lg shadow-black/20">
                                            ...
                                            <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/95 via-black/40 to-transparent translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-400 ease-out">
                                                <p className="text-[11px] text-white font-semibold truncate mb-1.5" title={photo.original_filename}>
                                                    {photo.original_filename}
                                                </p>
                                                <div className="flex items-center justify-between">
                                                    <span className="text-[10px] text-slate-400 font-medium">
                                                        {new Date(photo.created_at).toLocaleDateString(undefined, { day: 'numeric', month: 'short' })}
                                                    </span>
