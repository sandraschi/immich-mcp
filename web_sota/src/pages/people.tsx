import { Filter, Loader2, Search, User, Users } from "lucide-react";
...
export function People() {
    ...
    const fetchPeople = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          const msg =
            (body as { detail?: string }).detail || res.statusText || "Failed to fetch people";
          throw new Error(msg);
        }
  ...
  const filteredPeople = people.filter(
    (p) =>
      (p.name || "Unnamed").toLowerCase().includes(searchQuery.toLowerCase()) && p.assetCount > 1,
  );

  return (
    <div className="space-y-6 pb-20" data-testid="people-page">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white mb-2">People</h1>
          <p className="text-slate-400">
            Manage face recognition clusters and identify family & friends.
          </p>
        </div>
        ...
        {!loading && !error && filteredPeople.length === 0 && (
          <div className="flex flex-col items-center justify-center min-h-[400px] text-slate-500 py-12">
            ...
            <p className="text-sm opacity-60 mt-2">
              Try re-running face detection in the Immich dashboard.
            </p>
          </div>
        )}
          ...
          >
            {filteredPeople.map((person) =>
              viewMode === "grid" ? (
                <div
                  key={person.id}
                  className="group relative flex flex-col items-center gap-3 p-4 bg-slate-900/40 hover:bg-slate-800/60 rounded-2xl border border-slate-800/50 transition-all duration-300 cursor-pointer hover:-translate-y-1 shadow-lg border-b-2 hover:border-b-primary/50"
                >
                    ...
                    {person.thumbnailPath ? (
                      <img
                        ...
                        onError={(e) => {
                          ...
                          icon.innerHTML =
                            '<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-slate-600"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>';
                  ...
                  <div className="text-center w-full overflow-hidden">
                    <h3
                      className="text-sm font-bold text-white truncate px-2"
                ...
                <div
                  key={person.id}
                  className="flex items-center justify-between p-3 bg-slate-900/30 hover:bg-slate-800/50 rounded-xl border border-slate-800/50 transition-colors cursor-pointer group"
                >
