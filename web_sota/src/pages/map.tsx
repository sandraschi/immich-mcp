...
import { Navigation, Compass, Layers, Maximize, ZoomIn, Loader2, MapPin, Globe, Activity } from 'lucide-react';

export function Map() {
    const { data: features, isLoading, error } = useQuery<any[]>({
        ...
        <div className="min-h-[70vh] flex flex-col gap-6 pb-20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-400">Navigate your visual history through precise coordinate clusters.</p>
                </div>
                <div className="flex items-center gap-3">
                    <div className="px-4 py-2 bg-slate-900/50 border border-slate-800 rounded-xl flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold text-slate-300">
                            {isLoading ? 'Scanning metadata...' : `${totalAssets.toLocaleString()} Geotagged Points`}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="relative flex-1 bg-slate-900 rounded-2xl border border-slate-700 overflow-hidden shadow-2xl group min-h-[500px]">
                ...
                <div className="absolute top-4 right-4 flex flex-col gap-2 z-20">
                    <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-700/50 rounded-xl p-1 shadow-2xl">
                        <button title="Full Screen" className="p-2 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white transition-colors"><Maximize className="h-4 w-4" /></button>
                        <div className="h-px bg-slate-800 my-1 mx-2" />
                        <button title="Zoom In" className="p-2 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white transition-colors"><ZoomIn className="h-4 w-4" /></button>
                    </div>
                </div>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 z-10">
                    {isLoading ? (
                        <div className="flex flex-col items-center gap-4 bg-slate-900/90 backdrop-blur rounded-2xl border border-slate-700 p-8">
                            ...
                            <p className="text-sm font-bold text-slate-200 uppercase tracking-widest">Loading map data...</p>
                        </div>
                    ) : error ? (
                        ...
                        <div className="flex flex-col items-center gap-4 bg-slate-900/90 backdrop-blur rounded-2xl border border-slate-700 p-8 text-slate-300">
                            ...
                            <p className="max-w-xs text-xs text-slate-400">Photos with location data will appear here.</p>
                        </div>
                    ) : (
                        <div className="relative group/canvas w-full h-full flex items-center justify-center">
                            ...
                            <div className="grid grid-cols-12 grid-rows-8 gap-4 w-full h-full opacity-30">
                                {Array.from({ length: 48 }).map((_, i) => (
                                    <div
                                        key={i}
                                        className={`rounded-full shadow-[0_0_15px_rgba(59,130,246,0.3)] ${Math.random() > 0.7 ? 'bg-primary h-2 w-2 animate-pulse' : 'bg-slate-800 h-1 w-1'
                                            }`}
                                        style={{
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                <div className="p-8 bg-slate-900/60 backdrop-blur-2xl rounded-3xl border border-slate-700/50 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
                                    ...
                                    <h3 className="text-2xl font-black text-white mb-2 uppercase tracking-tight">Sync Complete</h3>
                                    <p className="text-slate-400 text-sm max-w-sm font-medium">
                                        Rendered {totalAssets.toLocaleString()} global features.
                                        Interactive drill-down active for authenticated clusters.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { label: 'Density Filter', value: 'High Accuracy', sub: 'EXIF Precision < 5m', icon: MapPin },
                    { label: 'Temporal Drift', value: 'Sync Active', sub: 'Auto-refresh enabled', icon: Activity },
                    { label: 'Metadata Audit', value: 'Validated', sub: '99.4% Coordinate Match', icon: Layers }
                ].map((stat, i) => (
                    <div key={i} className="bg-slate-900/40 p-5 rounded-2xl border border-slate-800/50 hover:bg-slate-900/60 transition-all cursor-crosshair group">
                        <div className="flex items-start justify-between mb-3">
                            <div className="text-[9px] font-black text-primary uppercase tracking-[0.2em]">{stat.label}</div>
                            <stat.icon className="h-4 w-4 text-slate-600 group-hover:text-primary transition-colors" />
                        </div>
                        <div className="text-xl font-black text-white">{stat.value}</div>
                        <div className="text-[10px] text-slate-500 mt-1 font-bold uppercase tracking-tight">{stat.sub}</div>
                    </div>
                ))}
            </div>
        </div>
