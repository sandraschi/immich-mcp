...
const BACKEND_NAME: &str = "immich-mcp-backend.exe";
const BACKEND_TAG: &str = "immich-mcp-backend-x86_64-pc-windows-msvc.exe";
const BACKEND_PORT: u16 = 10839;
...
fn resolve_bundled_backend(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    if let Ok(path) = app
        .path()
        .resolve("resources/immich-mcp-backend.exe", BaseDirectory::Resource)
    {
        tried.push(path.display().to_string());
        if path.exists() {
            return Ok(path);
...
fn free_port(port: u16) -> bool {
    ...
    #[cfg(windows)]
    {
        let img_kill = "Stop-Process -Name 'immich-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'immich-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM immich-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM immich-mcp-native.exe /T 2>$null";
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name immich-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM immich-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    if !free_port(BACKEND_PORT) {
        let msg = format!("Could not free port {BACKEND_PORT} after 120s - TIME_WAIT not cleared");
        log_line(&app, &msg);
        return Err(msg);
    }
