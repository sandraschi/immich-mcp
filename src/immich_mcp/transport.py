...
Standard module for MCP servers. Unified transport: STDIO, HTTP Streamable, legacy SSE.

Environment Variables:
    MCP_TRANSPORT: Transport mode (stdio, http, sse). Default: stdio
    MCP_HOST: Bind address for HTTP/SSE. Default: 127.0.0.1
    MCP_PORT: Port for HTTP/SSE. Default: 10839 (fleet 10700+; set MCP_PORT to override)
...
ENV_PORT = "MCP_PORT"  # default: 10839
...
def get_transport_config() -> dict:
    ...
    return {
        ...
        "port": int(os.getenv(ENV_PORT, "10839").strip()),
...
Environment Variables:
  ...
  {ENV_PORT}         Port number (default: 10839)
...
Examples:
  ...
  # HTTP mode (web apps)
  python -m {server_name.replace("-", "_")} --http --port 10839

  # Via environment
  MCP_TRANSPORT=http MCP_PORT=10839 python -m {server_name.replace("-", "_")}
""",
    )
    ...
    parser.add_argument("--port", type=int, default=None, help=f"Port to listen on (default: ${ENV_PORT} or 10839)")
