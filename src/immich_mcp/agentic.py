...
def register_agentic_tools():
    ...
    async def immich_help(ctx: Context, category: str | None = None) -> str:
        ...
        if category and category.lower() in help_data:
            await ctx.info(f"Retrieving help for category: {category}")
            return (
    ...
    async def agentic_immich_workflow(
        ...
        """
        await ctx.info(f"Starting agentic workflow: {workflow_prompt}")
    ...
    async def intelligent_photo_processing(
        ...
        """
        await ctx.info(f"Intelligent processing for {len(photos)} photos: {processing_goal}")
    ...
    async def conversational_immich_assistant(
        ...
        """
        await ctx.info(f"Processing conversational query: {user_query}")
        ...
        return sampling_result.text or ""
